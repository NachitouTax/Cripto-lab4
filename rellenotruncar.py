...
def adjust_key(key, required_length):
    # Truncar o completar la clave según sea necesario, sin reemplazo completo
    if len(key) < required_length:
        # Si la clave es demasiado corta, agregar bytes aleatorios al final
        additional_bytes = get_random_bytes(required_length - len(key))
        adjusted_key = key + additional_bytes
    elif len(key) > required_length:
        # Si la clave es demasiado larga, truncar a los últimos 24 bytes
